...
The final project for the Data Science Specialization with Johns Hopkins University and Coursera was to develop a natural language processing (NLP) application in R and deploying it using Shiny.io. Trained off blog posts, news articles and tweets, the application determines the most likely next word given an input string of words. This project developed in collaboration with SwiftKey, the maker of a similar text prediction algorithm, and the providers of the training corpuses.  
...
One difference between this implementation of the modified Kneser-Ney Smoothing algorithm and the one presented in the Chen and Goodman paper is that this implementation treats all n-gram lengths as the highest order n-gram when calculating. In the 1999 paper implementations, in sections 4.1.6-7, n-gram levels below the highest are smoothed by using the unique combination counts instead of corpus frequency:  
$$
...
$$  
It was not feasible (due to time constraints) to develop the lower-order model originally, but it would be interesting to compare the models in this task.  
